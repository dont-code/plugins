...
describe('TimeFieldsComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      imports: [BrowserModule, CalendarModule,TooltipModule, ReactiveFormsModule, FormsModule]
    })
    .compileComponents();
  });
...
@Component({
  template: `
    ...
    <form [formGroup]="form">
        <ng-container *ngTemplateOutlet="template(); context:{ fieldName:'TestDate'}"></ng-container>
    </form>
  `
})
class WrapperComponent implements OnInit{
  ...
  template (): TemplateRef<any> | null | undefined {
    const ret= this.appComponentRef?.providesTemplates('Date & Time').forFullEdit;
    return ret;
  }
